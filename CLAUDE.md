...
2. `docs/07-next-tasks.md` 를 읽습니다 — **하나의 우선순위 목록**입니다.
   ...
   **그것만으로 이어갈 수 있습니다**
3. **`docs/09-handover.md` 를 읽습니다** — 시스템 구조 · 데이터 계약 ·
   **이 프로젝트의 작업 리듬** · **함정 모음**(같은 데서 두 번 다치지 않게).
   며칠 만에 돌아왔거나 새 채팅창이면 **이것부터** 보세요
4. 더 자세한 배경이 필요하면 `docs/devlog/byeonsoyun.md` 맨 위 항목을 봅니다
   (특히 "막혔던 것 / 어떻게 풀었는지")
5. **지금 상태를 한 문단으로 말하고, `- [ ]` 중 맨 위 것 하나를 제안합니다.**
   여러 개를 동시에 벌이지 않습니다
6. 한 항목이 끝나면 `docs/07-next-tasks.md` 의 `- [ ]` 를 `- [x]` 로 바꾸고,
   **"★ 지난 세션이 끝난 자리" 표를 갱신하고**, 개발일지와 함께 커밋합니다

> **"나 뭐 하면 되는지 알려줘"** 나 **"docs 보고 바로 진행해줘"** 라고만 오면
> 되묻지 말고 위 1~5 를 그대로 하세요.

### 지금은 `- [ ]` 가 하나도 없습니다 (2026-08-28)

계획했던 개발 항목이 전부 끝났습니다. 그래서 4번에서 **집어 줄 "맨 위 하나" 가
없습니다.** 이럴 때는 아래 셋 중 하나를 제안하고, 사용자가 다른 것을 원하면 그걸 합니다.

| 무엇 | 어디에 적혀 있나 |
|---|---|
| **손으로 확인하기** (QR 을 폰으로 찍기 · 글 못 읽는 척하고 끝까지 가기 · **비행기 모드**) | `docs/10-checklist.md` |
| **알려진 한계 메우기** (댓글 수정·삭제 없음 · 기한 알림 없음 …) | 각 항목의 "남는 한계" |
| 확인하다 찾은 것 고치기 | `docs/10-checklist.md` 맨 아래 "확인 결과 적는 칸" |

> **E1(오프라인 우선 동작)은 2026-08-28 에 끝났습니다.** `src/sw.js` 가 화면 파일을
> 담아 둡니다. **배포할 때마다 `src/sw.js` 의 `VERSION` 을 올려야 합니다** —
> 안 올리면 옛 화면이 그대로 남아 "고쳤는데 안 고쳐진" 것으로 보입니다.
> 그리고 **새 화면이나 새 공용 파일을 만들면 `sw.js` 의 목록에 한 줄 더합니다**
> (`tests/test-offline.js` 가 `src/` 와 대조하니 빠뜨리면 검사가 잡습니다).

### 문서 지도 (어떤 상황에 무엇을 여나)

| 상황 | 문서 |
|---|---|
| 다음에 뭐 하지 | `docs/07-next-tasks.md` ★ 진행 상태의 정답 |
