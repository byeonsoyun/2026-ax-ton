...
// 영상 렌더링용 서버사이드 TTS. 계정/키가 필요한 Google Cloud TTS 대신, 무료·로컬 엔진을 쓴다
// (tools/README.md 참고). 언어별 커버리지가 다르므로 지원하지 않는 언어는 명시적으로 에러를
// 던진다 — 조용히 다른 언어로 대체하지 않는다 (PRD §4.2 원칙: 검증 없이 대충 넘기지 않는다).

export const VIDEO_TTS_SUPPORTED_LANGUAGES = ["ko", "vi"] as const;
...
async function synthesizeVietnameseWav(text: string, outPath: string): Promise<void> {
  const bin = process.env.PIPER_BIN_PATH;
  const model = process.env.PIPER_VI_MODEL_PATH;
  if (!bin || !model) throw new Error("PIPER_BIN_PATH / PIPER_VI_MODEL_PATH가 설정되지 않았습니다");
...
export async function synthesizeNarrationWav(
  ...
  if (lang === "vi") return synthesizeVietnameseWav(text, outPath);
